...
first = Sequential()
first.add(Embedding(vocabulary_size, embeds_size, input_length=max_sentence_length, mask_zero=True))

second = Sequential()
second.add(Embedding(vocabulary_size, embeds_size, input_length=max_sentence_length, mask_zero=True))

result = Sequential()
result.add(Merge([first, second], mode='concat'))
...
for epoch in range(epochs):
    ...
    result.fit([X_train_pad, X_train_pos_pad], y_train_1hot_pad, epochs = 1, batch_size = 5, validation_data=([X_dev_pad, X_dev_pos_pad], y_dev_1hot_pad))

    prd = result.predict_classes([X_dev_pad, X_dev_pos_pad])
    prdtest = result.predict_classes([X_test_pad, X_test_pos_pad])
...
prd = bestmodel.predict_classes([X_test_pad, X_test_pos_pad])
idx = 0
idx2 = 0
length = 0
